...
ifdef(`BUILD_FOR_GPU',`
dnl	dnl // put null entries here for these funcs with no GPU implementation
dnl	ifdef(`BUILD_FOR_CUDA',`
dnl	NULL_ARR(vuni,FVUNI),
dnl	',`
dnl	RFLT_SAME_PREC_ARR(vuni,FVUNI),
dnl	')
...
REAL_INT_ARR_NO_BITMAP(visprint,FVISPRINT),
