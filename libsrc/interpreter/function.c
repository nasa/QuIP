...
#ifdef FOOBAR
void add_sizable(QSP_ARG_DECL  Item_Type *itp,Size_Functions *sfp,
			Item *(*lookup)(QSP_ARG_DECL  const char *))
{
	if( siz_icp == NO_ITEM_CLASS )
...
static Item *_check_sizable(QSP_ARG_DECL  const char *name)
{
	if( siz_icp == NO_ITEM_CLASS )
		init_sizable_class(SINGLE_QSP_ARG);

	return( check_member(QSP_ARG  siz_icp,name) );
}

Item *check_sizable(QSP_ARG_DECL  const char *name)
{
	Item *ip;

	ip = _check_sizable(QSP_ARG  name);
#ifdef BUILD_FOR_OBJC
	if( ip == NULL )
		ip = (__bridge Item *) check_ios_sizable(QSP_ARG  name);
...
static
#endif /* ! BUILD_FOR_OBJC */
	Size_Functions *get_sizable_functions(QSP_ARG_DECL  Item *ip)
{
	Member_Info *mip;
...
//		sprintf(ERROR_STRING,
//			"CAUTIOUS:  get_sizable_functions %s, missing member info #2",
//			ip->item_name);
//		ERROR1(ERROR_STRING);
//	}
//#endif /* CAUTIOUS */
...
#ifdef CAUTIOUS
/* This check now done with assertion... */
#define MEMBER_CAUTIOUS_CHECK(type_stem)				\
	if( mip == NO_MEMBER_INFO ){					\
		sprintf(ERROR_STRING,					\
			"CAUTIOUS:  get_%s_functions %s: missing member info!?", \
			#type_stem,ip->item_name);			\
		ERROR1(ERROR_STRING);					\
	}
#else // ! CAUTIOUS
#define MEMBER_CAUTIOUS_CHECK(type_stem)
