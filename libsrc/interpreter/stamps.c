...
char VersionId_interpreter_stamps[] = QUIP_VERSION_STRING;
...
static void show_one(Stamped_Char *scp, int max_per_line)
{
	int i=0;
	long sec, msec;
	...
	if( zero_set ){
		...
		msec = floor((scp->sc_tv.tv_usec - zero_tv.tv_usec)/1000.0);
	} else {
		sec = scp->sc_tv.tv_sec ;
		msec = floor(scp->sc_tv.tv_usec/1000);
	}
...
static COMMAND_FUNC( show_stamps )
{
	...
	while(np!=NO_NODE){
		...
		show_one(scp,DEFAULT_MAX_PER_LINE);
		np=np->n_next;
	}
}
...
static void decode_file(int fd, int max_per_line)
{
	int reading_file=1;
	int count,actual;
...
int ntot=0;
	...
	while( reading_file ){
		...
		if( (actual=read(fd,(char *)&stmp_tbl[0],count)) < 0 ){
			tell_sys_error("read");
			NWARN("error reading timestamp data");
sprintf(DEFAULT_ERROR_STRING,"Read error after reading %d total characters",ntot);
advise(DEFAULT_ERROR_STRING);
			return;
		} else if( actual == 0 ){
			...
			sprintf(DEFAULT_ERROR_STRING,"Requested %d timestamp chars, %d actually read",
				count,actual);
...
ntot+=count;
		...
		if( (actual=read(fd,(char *)&char_offset,count)) < 0 ){
			tell_sys_error("read");
			NWARN("error reading char count");
sprintf(DEFAULT_ERROR_STRING,"Read error after reading %d total characters",ntot);
advise(DEFAULT_ERROR_STRING);
			return;
		} else if( actual != count ){
			sprintf(DEFAULT_ERROR_STRING,"Requested %d charcount chars, %d actually read",
				count,actual);
			NWARN(DEFAULT_ERROR_STRING);
sprintf(DEFAULT_ERROR_STRING,"Read error after reading %d total characters",ntot);
advise(DEFAULT_ERROR_STRING);
...
ntot+=count;
		...
		if( (actual=read(fd,&char_buf,count)) < 0 ){
			tell_sys_error("read");
			NWARN("error reading char buffer");
sprintf(DEFAULT_ERROR_STRING,"Read error after reading %d total characters",ntot);
advise(DEFAULT_ERROR_STRING);
			return;
		} else if( actual != count ){
			sprintf(DEFAULT_ERROR_STRING,"Requested %d data chars, %d actually read",
				count,actual);
			NWARN(DEFAULT_ERROR_STRING);
sprintf(DEFAULT_ERROR_STRING,"Read error after reading %d total characters",ntot);
advise(DEFAULT_ERROR_STRING);
...
ntot+=count;
		...
		while(i<MAX_TIME_STAMPS && stmp_tbl[i].sc_n > 0 ){
			...
			show_one(&stmp_tbl[i],DEFAULT_MAX_PER_LINE);
...
Command sc_ctbl[]={
{ "zero",	set_zero,	"set time zero"			},
{ "read",	rd_stamps,	"read available characters"	},
{ "async",	do_set_async,	"set/clear async flag"		},
{ "halt",	do_stamp_halt,	"halt async acquisition"	},
{ "status",	do_status,	"report status of async acquisition"	},
{ "show",	show_stamps,	"display timestamped chars"	},
{ "decode",	do_decode,	"decode stored stream file"	},
{ "quit",	popcmd,		"exit submenu"			},
{ NULL_COMMAND							}
};

COMMAND_FUNC( stamp_menu )
{
	PUSHCMD(sc_ctbl,"stamps");
}
