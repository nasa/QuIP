...
int _set_camera_framerate(QSP_ARG_DECL  PGR_Cam *pgcp, dc1394framerate_t framerate )
{
	/*
	IsoFlowCheck(&state);
	*/
	if( dc1394_video_set_framerate(pgcp->pc_cam_p, framerate ) != DC1394_SUCCESS ){
		warn("Could not set framerate");
		return -1;
	}
	/*
	IsoFlowResume(&state);
...
int _power_on_camera(QSP_ARG_DECL  PGR_Cam *pgcp)
{
#ifdef HAVE_LIBDC1394
	if( dc1394_camera_set_power(pgcp->pc_cam_p, DC1394_ON) != DC1394_SUCCESS ){
		warn("Could not set camera 'on'");
		return(-1);
	}
#endif
	return 0;
...
int _power_off_camera(QSP_ARG_DECL  PGR_Cam *pgcp)
{
#ifdef HAVE_LIBDC1394
	if( dc1394_camera_set_power(pgcp->pc_cam_p, DC1394_OFF) != DC1394_SUCCESS ){
		warn("Could not set camera 'off'");
		return -1;
	}
#endif
	return 0;
...
int _set_camera_trigger_mode(QSP_ARG_DECL  PGR_Cam *pgcp, dc1394trigger_mode_t mode)
{
	if( dc1394_external_trigger_set_mode(pgcp->pc_cam_p, mode) != DC1394_SUCCESS ){
		warn("Could not set trigger mode");
		return -1;
	}
	return 0;
	/*
...
int _set_camera_trigger_source(QSP_ARG_DECL  PGR_Cam *pgcp, dc1394trigger_source_t source)
{
	if( dc1394_external_trigger_set_source(pgcp->pc_cam_p, source) != DC1394_SUCCESS ){
		warn("Could not set trigger source");
		return -1;
	}
	return 0;
	/*
...
int _set_camera_temperature(QSP_ARG_DECL  PGR_Cam *pgcp, int temp)
{
#ifdef HAVE_LIBDC1394
	if( dc1394_feature_temperature_set_value(pgcp->pc_cam_p,temp) != DC1394_SUCCESS ){
		//warn("set_camera_temperature not implemented");
		return -1;
	}
#endif
	return 0;
...
int _set_camera_white_balance(QSP_ARG_DECL  PGR_Cam *pgcp, int wb)
{
#ifdef HAVE_LIBDC1394
	if( dc1394_feature_whitebalance_set_value(pgcp->pc_cam_p,wb, 0 /* old value? */ ) != DC1394_SUCCESS ){
		warn("Could not set B/U white balance");
		return -1;
	}
#endif
	return 0;
...
int _set_camera_white_shading(QSP_ARG_DECL  PGR_Cam *pgcp, int val)
{
#ifdef HAVE_LIBDC1394
	if( dc1394_feature_whiteshading_set_value(pgcp->pc_cam_p,val,0,0/*BUG what are these args*/)!=DC1394_SUCCESS){
		warn("Could not set white shading / blue channel");
		return -1;
	}
#endif
	return 0;
...
#ifdef HAVE_LIBDC1394
int _set_iso_speed(QSP_ARG_DECL  PGR_Cam *pgcp, dc1394speed_t speed)
{
...
int _set_camera_bmode(QSP_ARG_DECL  PGR_Cam *pgcp, int bmode_true)
{
	...
	if( bmode_true ){
		...
		if( dc1394_video_set_operation_mode(pgcp->pc_cam_p, DC1394_OPERATION_MODE_LEGACY) != DC1394_SUCCESS ){
			warn("unable to set 1394 legacy mode");
			return -1;
		}
	}
#endif
