...
char VersionId_lutmenu_bitmenu[] = QUIP_VERSION_STRING;
...
Command bitp_ctbl[]={
{ "ncomps",	do_set_ncomps,	"set number of image components"	},
{ "amplitudes",	do_bitplanes,	"set image component amplitudes"	},
{ "base_index",	do_set_base,	"set base index for image components"	},
{ "nlevels",	do_lvlspercomp,	"specify number of levels per image component"},
#ifdef FOOBAR
{ "drift_plane",do_drift_plane,	"plane drifting"			},
#endif
{ "vectors",	do_set_bitvecs,	"specify bitplane color vectors"	},
{ "white",	do_setwhite,	"set white point for modulation"	},
{ "sine",	do_sine_mod,	"sinusoidal modulation"			},
#ifndef MAC
...
COMMAND_FUNC( bitmenu )
{
	PUSHCMD(bitp_ctbl,BITP_MENU_NAME);
}
