...
// BUG - this routine uses a fixed size buffer.  That is OK until
// we encounter something (such as a pathname) that is longer than
// the max space allocated.  This happened (barely) when running
// in the iOS simulator.  The not-so-ideal solution is to just make
// the buffer a bit bigger...

// This routine is a bit odd - it returns TRUE if there's
// an error.  This is consistent with the convention of returning
// 0 for success, but the error return should perhaps be -1?
// Also, we might want to return different error codes:
// -1 for no string, -2 for insufficient buffer space...
