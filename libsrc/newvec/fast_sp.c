...
char VersionId_newvec_fast_sp[] = QUIP_VERSION_STRING;
...
#define SIMD_OBJ_FUNC_3( name )						\
									\
void simd_obj_##name(Vec_Obj_Args *oap)					\
{									\
	if( (!IS_CONTIGUOUS(oap->oa_dest)) ||				\
		(!IS_CONTIGUOUS(oap->oa_1)) ||				\
		(!IS_CONTIGUOUS(oap->oa_2)) || 				\
		( ! use_sse_extensions ) ){				\
		sp_obj_##name(oap);					\
	} else {							\
		/* make sure that addresses are aligned */		\
		if( NOT_ALIGNED(oap->oa_dest->dt_data) ||		\
		    NOT_ALIGNED(oap->oa_1->dt_data) ||			\
		    NOT_ALIGNED(oap->oa_2->dt_data) ){			\
NWARN("data vectors must be aligned on 16 byte boundary for SSE acceleration");\
			sp_obj_##name(oap);				\
		} else {						\
			simd_vec_##name((float *)oap->oa_dest->dt_data,	\
				(float *)oap->oa_1->dt_data,		\
				(float *)oap->oa_2->dt_data,		\
				oap->oa_dest->dt_n_mach_elts);		\
		}							\
	}								\
}
...
#define SIMD_2_VEC_SCALAR( normfunc, sse_func )				\
	...
	if( (!IS_CONTIGUOUS(oap->oa_dest)) ||				\
		...
		float sv[4];						\
									\
		sv[0]=sv[1]=sv[2]=sv[3]= * ( (float *) oap->oa_svp[0] );\
									\
		sse_func((float *)oap->oa_dest->dt_data,		\
			(float *)oap->oa_1->dt_data,sv,			\
			oap->oa_dest->dt_n_mach_elts);			\
	}

void simd_vsadd(Vec_Obj_Args *oap) { SIMD_2_VEC_SCALAR( sp_obj_rvsadd, _simd_vsadd ) }
void simd_vssub(Vec_Obj_Args *oap) { SIMD_2_VEC_SCALAR( sp_obj_rvssub, _simd_vssub ) }
void simd_vsmul(Vec_Obj_Args *oap) { SIMD_2_VEC_SCALAR( sp_obj_rvsmul, _simd_vsmul ) }
void simd_vsdiv(Vec_Obj_Args *oap) { SIMD_2_VEC_SCALAR( sp_obj_rvsdiv, _simd_vsdiv ) }
