...
void push_nav(QSP_ARG_DECL  Gen_Win *gwp)
{
	...
	NSArray *a = [ root_view_controller viewControllers];
fprintf(stderr,"push_nav:  root view controller has %d view controllers\n",a.count);
	int i;
...
NSArray *a = [ root_view_controller viewControllers];
fprintf(stderr,"pop_nav %d:  before popping, root view controller has %d view controllers\n",n_levels,a.count);
...
#ifdef FOOBAR
	while( (n_levels--) > 1 ){
fprintf(stderr,"pop_nav before popping, n_levels = %d\n",n_levels);
fprintf(stderr,"pop_nav calling popViewControolerAnimated:NO...\n");
...
a = [ root_view_controller viewControllers];
fprintf(stderr,"pop_nav:  after extra pops, root view controller has %d view controllers\n",a.count);
#endif // FOOBAR
	if( n_levels > 1 ){
		if( n_levels >= a.count ){
	...
	/* qvc = (quipViewController *) */
fprintf(stderr,"pop_nav calling popViewControolerAnimated:YES...\n");
...
a = [ root_view_controller viewControllers];
fprintf(stderr,"pop_nav:  after popping, root view controller has %d view controllers\n",a.count);
}
