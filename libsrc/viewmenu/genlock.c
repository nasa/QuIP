...
char VersionId_viewmenu_genlock[] = QUIP_VERSION_STRING;
...
int wait_til_vblank_transition(int fd);


static pthread_mutex_t genlock_mutex;
static ParPort *the_ppp=NO_PARPORT;
static pthread_t vboard_thr, pport_thr;
static pthread_t gl_thr;
static struct timeval *tvp=NULL,tv0,tv1;


long compute_latency(struct timeval *later_tvp, struct timeval *earlier_tvp)
{
	long d_secs, d_usecs;
...
void *pport_logger(void *argp)
{
	...
	while(n--){
		s=read_til_transition(ppip->ppi_ppp,8);
		*lp = delta_usecs();
		lp += ppip->ppi_inc;
	}
	return(NULL);
...
static void decrease_frame_time(FB_Info *fbip)
{
	/* get_genlock_mutex(); */
...
static void increase_frame_time(FB_Info *fbip)
{
	/* get_genlock_mutex(); */
	if (ioctl(fbip->fbi_fd,FBIOGET_VSCREENINFO, &fbip->fbi_var_info)<0) {
		perror("ioctl error getting variable screeninfo");
...
static void adjust_genlock(Genlock_Info *glip,int index)
{
...
COMMAND_FUNC( test_parport )
{
	...
	if( fbip == NO_FBI || dp == NO_OBJ ) return;

	INSIST_RAM(dp,"test_parport")

	if( dp->dt_prec != PREC_UDI ){
		sprintf(error_string,"latency vector %s (%s) should have precision %s",
			dp->dt_name,name_for_prec(dp->dt_prec),name_for_prec(PREC_UDI));
		warn(error_string);
		return;
	}
	...
	if( ! IS_CONTIGUOUS(dp) ){
		sprintf(error_string,"latency vector %s should be contiguous",dp->dt_name);
		warn(error_string);
		return;
	}

	n = dp->dt_n_mach_elts/2;
	lp = (u_long *) dp->dt_data;
	...
	if( the_ppp == NO_PARPORT ){
		the_ppp=open_parport(QSP_ARG  NULL);	/* use default */
		if( the_ppp == NO_PARPORT ) return;
	}

	ppi1.ppi_ppp = the_ppp;
	ppi1.ppi_buf = lp;
	ppi1.ppi_count = n;
	ppi1.ppi_inc = 2;
	...
	pthread_create(&pport_thr,&attr1,pport_logger,&ppi1);
...
int init_genlock(SINGLE_QSP_ARG_DECL)
{
	...
	for(i=0;i<n;i++)
		if( gli1.gli_fbip[i] == NO_FBI ) return -1;

	if( the_ppp == NO_PARPORT ){
		the_ppp=open_parport(QSP_ARG  NULL);	/* use default */
		if( the_ppp == NO_PARPORT ) return -1;
	}
...
COMMAND_FUNC( report_genlock_status )
{
	FB_Info *fbip;
	int i,index;
	float l2,l3;
...
static void start_fb_threads(int n_frame_buffers,int* fd_arr)
{
	int i;
...
COMMAND_FUNC( do_fbpair_monitor )
{
	...
	if( fbip1 == NO_FBI || fbip2 == NO_FBI ) return;
