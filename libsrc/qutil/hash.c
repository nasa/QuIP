...
char VersionId_qutil_hash[] = QUIP_VERSION_STRING;
...
void tell_hash_stats(Hash_Tbl *htp)
		/* hash table */
{
	sprintf(msg_str,"\tStatistics for hash table \"%s\":",
		htp->ht_name);
	prt_msg(msg_str);
	sprintf(msg_str,"\tsize %ld, %ld entries",
	...
	sprintf(msg_str,"\t%ld searches",htp->ht_searches);
	*/
	sprintf(msg_str,"\t%g searches",htp->ht_searches);
	prt_msg(msg_str);

	if( htp->ht_searches > 0 ){
		double d;

		sprintf(msg_str,"\t%g collisions",htp->ht_collisions);
		prt_msg(msg_str);
		d=(double)htp->ht_collisions;
		d /= (double) htp->ht_searches;
		sprintf(msg_str,"\t%f collisions/search",d);
		prt_msg(msg_str);
	}
#endif
	if( htp->ht_removals > 0 ){
		sprintf(msg_str,"\t%ld removals",htp->ht_removals);
		prt_msg(msg_str);
		sprintf(msg_str,"\t%ld items checked",htp->ht_checked);
		prt_msg(msg_str);
		sprintf(msg_str,"\t%ld items moved",htp->ht_moved);
		prt_msg(msg_str);
	}
