...
#define OBJ_ARG_CHK_SRC1 /* OBJ_ARG_CHK(src1_dp,"first source") */	assert( src1_dp != NO_OBJ );
#define OBJ_ARG_CHK_SRC2 /* OBJ_ARG_CHK(src2_dp,"second source") */	assert( src2_dp != NO_OBJ );
#define OBJ_ARG_CHK_SRC3 /* OBJ_ARG_CHK(src3_dp,"third source") */	assert( src3_dp != NO_OBJ );
#define OBJ_ARG_CHK_SRC4 /* OBJ_ARG_CHK(src4_dp,"fourth source") */	assert( src4_dp != NO_OBJ );
...
//#define EXTRA_DECLS_T1	std_type r,theta,arg;
//#define EXTRA_DECLS_T2	std_type std_tmp;
//#define EXTRA_DECLS_T3	std_type std_tmp, tmp_denom;
...
#define FAST_INIT_DBM		which_bit = VA_BIT0(vap);		\
				dbm_ptr=VA_DEST_PTR(vap);	\
				FAST_INIT_COUNT

#define FAST_INIT_SBM		which_bit = VA_BIT0(vap);		\
				sbm_ptr=VA_SRC_PTR(vap,4);
...
#define ONE_VEC_2SCALAR_METHOD( name, statement, gpu_stat )		\
...
#define PROJECTION_METHOD_2( name, init_statement, statement, gpu_expr )	\
								\
OBJ_METHOD_DECL(name)						\
{								\
	DECL_VEC_ARGS_STRUCT(name)				\
...
#define PROJECTION_METHOD_IDX_2( name, init_statement, statement, gpu_s1, gpu_s2 )	\
								\
OBJ_METHOD_DECL(name)						\
{								\
	DECL_VEC_ARGS_STRUCT(name)				\
...
#define qscalar1_val	VA_SCALAR_VAL_STDQ(vap,0)
#define qscalar2_val	VA_SCALAR_VAL_STDQ(vap,1)
#define qscalar3_val	VA_SCALAR_VAL_STDQ(vap,2)
#define count		VA_DEST_COUNT(vap)		// ->va_szi_p->szi_dst_dim
#define s1_count	VA_SRC1_COUNT(vap)		// ->va_szi_p->szi_src_dim[0]
#define s2_count	VA_SRC2_COUNT(vap)		// ->va_szi_p->szi_src_dim[1]
#define dbminc		VA_DEST_INC(vap)		// ->va_spi_p->spi_dst_incr
#define sbminc		VA_SRC5_INC(vap)		// ->va_spi_p->spi_src_incr[4]
#define dinc		VA_DEST_INC(vap)		// ->va_spi_p->spi_dst_incr
#define s1inc		VA_SRC1_INC(vap)		// ->va_spi_p->spi_src_incr[0]
#define s2inc		VA_SRC2_INC(vap)		// ->va_spi_p->spi_src_incr[1]
#define s3inc		VA_SRC3_INC(vap)		// ->va_spi_p->spi_src_incr[2]
#define s4inc		VA_SRC4_INC(vap)		// ->va_spi_p->spi_src_incr[3]
